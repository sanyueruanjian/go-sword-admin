...
type SysQuartzJob struct {
	...
	IsPause           uint8     `json:"is_pause"`              //状态：0暂停、1启用
	...
	PauseAfterFailure uint8     `json:"pause_after_failure"`   //任务失败后是否暂停,0是暂停，1是不暂停
	CreateBy          int       `json:"create_by"`             //创建者
	UpdateBy          int       `json:"update_by"`             //更新者
	CreateTime        time.Time `json:"create_time"`           //创建日期
	UpdateTime        time.Time `json:"update_time"`           //更新时间
	IsDeleted         uint8     `json:"is_deleted"`            //逻辑删除：0启用（默认）、1删除
}
